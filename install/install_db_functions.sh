...
function set_path () {
    ...
    then
        ...
        export SCRIPT_PATH=$APP_DIR/data
        export LOG_PATH=$APP_DIR/var/log/geonature-db
    fi
}
...
function get_sql () {
    ...
    then
        ...
        wget https://raw.githubusercontent.com/PnEcrins/UsersHub/$usershub_release/data/usershub.sql -P $SCRIPT_PATH/imported/utilisateurs
        wget https://raw.githubusercontent.com/PnEcrins/UsersHub/$usershub_release/data/usershub-data.sql -P $SCRIPT_PATH/imported/utilisateurs
        wget https://raw.githubusercontent.com/PnEcrins/UsersHub/$usershub_release/data/usershub-dataset.sql -P $SCRIPT_PATH/imported/utilisateurs
        wget https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/adds_for_usershub.sql -P $SCRIPT_PATH/imported/utilisateurs
    fi
...
function create_database () {
    ...
    export PGPASSWORD=$POSTGRES_PASSWORD;psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/imported/utilisateurs/usershub.sql &>> $LOG_PATH/install_db.log
    export PGPASSWORD=$POSTGRES_PASSWORD;psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/imported/utilisateurs/usershub-data.sql &>> $LOG_PATH/install_db.log
    if [[ $ADD_USERSHUB_SAMPLE_DATA = "true" ]]
    then
        write_log "Insertion of data for usershub..."
        # insert geonature data for usershub
        export PGPASSWORD=$POSTGRES_PASSWORD;psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/imported/utilisateurs/usershub-dataset.sql &>> $LOG_PATH/install_db.log
        # insert taxhub data for usershub
        export PGPASSWORD=$POSTGRES_PASSWORD;psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/imported/utilisateurs/adds_for_usershub.sql &>> $LOG_PATH/install_db.log
    fi
