...
function get_sql () {
    wget -N https://raw.githubusercontent.com/PnEcrins/UsersHub/$USERSHUB_RELEASE/data/usershub.sql -P $GN_SQL_SCRIPTS_DIR/imported/users
    wget -N https://raw.githubusercontent.com/PnEcrins/UsersHub/$USERSHUB_RELEASE/data/usershub-data.sql -P $GN_SQL_SCRIPTS_DIR/imported/users
    wget -N https://raw.githubusercontent.com/PnEcrins/UsersHub/$USERSHUB_RELEASE/data/usershub-dataset.sql -P $GN_SQL_SCRIPTS_DIR/imported/users
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/adds_for_usershub.sql -P $GN_SQL_SCRIPTS_DIR/imported/users
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/inpn/data_inpn_taxhub.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N http://geonature.fr/data/inpn/taxonomie/TAXREF_INPN_v11.zip -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N http://geonature.fr/data/inpn/taxonomie/ESPECES_REGLEMENTEES_v11.zip -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N http://geonature.fr/data/inpn/taxonomie/LR_FRANCE_20160000.zip -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/taxhubdb.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/taxhubdata.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/taxhubdata_atlas.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/materialized_views.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
    wget -N https://geonature.fr/data/inpn/habitats/HABREF_50.zip -P $GN_SQL_SCRIPTS_DIR/imported/habitats
    wget -N https://raw.githubusercontent.com/PnX-SI/Habref-api-module/$HABREF_API_RELEASE/src/pypn_habref_api/data/habref.sql -P $GN_SQL_SCRIPTS_DIR/imported/habitats
    wget -N https://raw.githubusercontent.com/PnX-SI/Habref-api-module/$HABREF_API_RELEASE/src/pypn_habref_api/data/data_inpn_habref.sql -P $GN_SQL_SCRIPTS_DIR/imported/habitats
    wget -N --no-cache --no-cookies https://raw.githubusercontent.com/PnX-SI/Nomenclature-api-module/$NOMENCLATURE_RELEASE/data/nomenclatures.sql -P $GN_SQL_SCRIPTS_DIR/imported/nomenclatures
    wget -N --no-cache --no-cookies https://raw.githubusercontent.com/PnX-SI/Nomenclature-api-module/$NOMENCLATURE_RELEASE/data/data_nomenclatures.sql -P $GN_SQL_SCRIPTS_DIR/imported/nomenclatures
    wget -N https://raw.githubusercontent.com/PnX-SI/Nomenclature-api-module/$NOMENCLATURE_RELEASE/data/nomenclatures_taxonomie.sql -P $GN_SQL_SCRIPTS_DIR/imported/nomenclatures
    wget -N https://raw.githubusercontent.com/PnX-SI/Nomenclature-api-module/$NOMENCLATURE_RELEASE/data/data_nomenclatures_taxonomie.sql -P $GN_SQL_SCRIPTS_DIR/imported/nomenclatures
    wget -N http://geonature.fr/data/ign/communes_fr_admin_express_2019-01.zip -P $GN_SQL_SCRIPTS_DIR/imported/geo
    wget -N https://geonature.fr/data/inpn/layers/2019/inpn_grids.zip -P $GN_SQL_SCRIPTS_DIR/imported/geo
    wget -N http://geonature.fr/data/ign/BDALTIV2_2-0_250M_ASC_LAMB93-IGN69_FRANCE_2017-06-21.zip -P $GN_SQL_SCRIPTS_DIR/imported/geo
    wget -N https://geonature.fr/data/inpn/sensitivity/181201_referentiel_donnes_sensibles.csv -P /tmp/geonature
    wget -N https://raw.githubusercontent.com/PnX-SI/TaxHub/$TAXHUB_RELEASE/data/taxhubdata_taxons_example.sql -P $GN_SQL_SCRIPTS_DIR/imported/taxonomie
}
...
function database_exists () {
    ...
    then
        ...
        sudo -u postgres -s -- psql -p $GN_POSTGRES_PORT -tAl | grep -q "^$1|"
        return $?
    fi
}
...
function create_database () {
    ...
    sudo -n -u postgres -s createdb -p $GN_POSTGRES_PORT -O $GN_POSTGRES_USER $GN_POSTGRES_DB -T template0 -E UTF-8 -l $GN_POSTGRES_LOCALE
    write_log "Adding PostGIS and other use PostgreSQL extensions"
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c 'CREATE EXTENSION IF NOT EXISTS postgis;' &>> $GN_LOG_DIR/install_db.log
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c 'CREATE EXTENSION IF NOT EXISTS hstore;' &>> $GN_LOG_DIR/install_db.log
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c 'CREATE EXTENSION IF NOT EXISTS pg_trgm;' &>> $GN_LOG_DIR/install_db.log
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c 'CREATE EXTENSION IF NOT EXISTS "uuid-ossp";' &>> $GN_LOG_DIR/install_db.log
    ...
    write_log 'GRANT'
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -v MYPGUSER=$GN_POSTGRES_USER -d $GN_POSTGRES_DB -f $GN_SQL_SCRIPTS_DIR/grant.sql &>> $GN_LOG_DIR/install_db.log
    ...
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -f /tmp/taxhub/data_inpn_taxhub.sql &>> $GN_LOG_DIR/install_db.log
    ...
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB  -f $GN_SQL_SCRIPTS_DIR/imported/habitats/data_inpn_habref.sql &>> $GN_LOG_DIR/install_db.log
    ...
    sed -i "s/MYDEFAULTLANGUAGE/$GN_NOMENCLATURE_LANGUAGE/g" $GN_SQL_SCRIPTS_DIR/imported/nomenclatures/data_nomenclatures.sql
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -f $GN_SQL_SCRIPTS_DIR/imported/nomenclatures/data_nomenclatures.sql  &>> $GN_LOG_DIR/install_db.log
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -f $GN_SQL_SCRIPTS_DIR/imported/nomenclatures/data_nomenclatures_taxonomie.sql  &>> $GN_LOG_DIR/install_db.log

    # Commons schema
    write_log "Creating 'commons' schema..."
    # cp -P -u $GN_SQL_SCRIPTS_DIR/core/commons.sql /tmp/geonature/commons.sql
    # sed -i "s/MYLOCALSRID/$GN_LOCAL_SRID/g" /tmp/geonature/commons.sql
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -v MYLOCALSRID=$GN_LOCAL_SRID -f $GN_SQL_SCRIPTS_DIR/core/commons.sql  &>> $GN_LOG_DIR/install_db.log
    ...
    # sed -i "s/MYLOCALSRID/$GN_LOCAL_SRID/g" /tmp/geonature/ref_geo.sql
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -v MYLOCALSRID=$GN_LOCAL_SRID -f $GN_SQL_SCRIPTS_DIR/core/ref_geo.sql  &>> $GN_LOG_DIR/install_db.log
    ...
    then
        ...
        sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -f /tmp/geonature/fr_municipalities.sql &>> $GN_LOG_DIR/install_db.log
        write_log "Restore $GN_POSTGRES_USER owner"
        sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "ALTER TABLE ref_geo.temp_fr_municipalities OWNER TO $GN_POSTGRES_USER;" &>> $GN_LOG_DIR/install_db.log
        ...
        sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "DROP TABLE ref_geo.temp_fr_municipalities;" &>> $GN_LOG_DIR/install_db.log
    fi
    ...
    then
        ...
        sudo -n -u postgres -s psql -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -f /tmp/geonature/inpn_grids.sql &>> $GN_LOG_DIR/install_db.log
        write_log "Restore $GN_POSTGRES_USER owner"
        sudo -n -u postgres -s psql -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "ALTER TABLE ref_geo.temp_grids_1 OWNER TO $GN_POSTGRES_USER;" &>> $GN_LOG_DIR/install_db.log
        sudo -n -u postgres -s psql -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "ALTER TABLE ref_geo.temp_grids_5 OWNER TO $GN_POSTGRES_USER;" &>> $GN_LOG_DIR/install_db.log
        sudo -n -u postgres -s psql -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "ALTER TABLE ref_geo.temp_grids_10 OWNER TO $GN_POSTGRES_USER;" &>> $GN_LOG_DIR/install_db.log
    ...
    then
        ...
        #gdalwarp -t_srs EPSG:$GN_LOCAL_SRID /tmp/geonature/BDALTIV2_250M_FXX_0098_7150_MNT_LAMB93_IGN69.asc /tmp/geonature/dem.tif &>> $GN_LOG_DIR/install_db.log
        export PGPASSWORD=$GN_POSTGRES_PASSWORD;raster2pgsql -s $GN_LOCAL_SRID -c -C -I -M -d -t 5x5 /tmp/geonature/BDALTIV2_250M_FXX_0098_7150_MNT_LAMB93_IGN69.asc ref_geo.dem|psql -v ON_ERROR_STOP=1 -h $GN_POSTGRES_HOST -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB  &>> $GN_LOG_DIR/install_db.log
    	#echo "Refresh DEM spatial index. This may take a few minutes..."
        sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "REINDEX INDEX ref_geo.dem_st_convexhull_idx;" &>> $GN_LOG_DIR/install_db.log
        if [ $REFGEO_VECTORISE_DEM = "true" ];
        then
            write_log "Vectorisation of DEM raster. This may take a few minutes..."
            sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "INSERT INTO ref_geo.dem_vector (geom, val) SELECT (ST_DumpAsPolygons(rast)).* FROM ref_geo.dem;" &>> $GN_LOG_DIR/install_db.log
            write_log "Refresh DEM vector spatial index. This may take a few minutes..."
            sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -c "REINDEX INDEX ref_geo.index_dem_vector_geom;" &>> $GN_LOG_DIR/install_db.log
        fi
    fi
    ...
    # sed -i "s/MYLOCALSRID/$GN_LOCAL_SRID/g" /tmp/geonature/synthese.sql
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -v MYLOCALSRID=$GN_LOCAL_SRID -f $GN_SQL_SCRIPTS_DIR/core/synthese.sql  &>> $GN_LOG_DIR/install_db.log
    ...
    export PGPASSWORD=$GN_POSTGRES_PASSWORD;psql -h $GN_POSTGRES_HOST -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -U $GN_POSTGRES_USER -d $GN_POSTGRES_DB -v MYLOCALSRID=$GN_LOCAL_SRID -f $GN_SQL_SCRIPTS_DIR/core/monitoring.sql  &>> $GN_LOG_DIR/install_db.log
    ...
    sudo -n -u postgres -s psql -p $GN_POSTGRES_PORT -v ON_ERROR_STOP=1 -d $GN_POSTGRES_DB -f $GN_SQL_SCRIPTS_DIR/core/sensitivity_data.sql &>> $GN_LOG_DIR/install_db.log
